/*
 *  target_dec_app.c - hddlsrc ! <decoder> ! hddlsink
 *
...
static GOptionEntry entries[] = {
  ...
  {"port", 'p', 0, G_OPTION_ARG_INT, &port,
      "Port to listen to.", "P"},
  {"decoder", 'd', 0, G_OPTION_ARG_STRING, &codec,
      "Decoder codec to be used", "codec"},
...
main (int argc, char *argv[])
{
  ...
  if (connection_mode == NULL || port == -1 || codec == NULL || channel_num == -1) {
  ...
  if (!g_ascii_strncasecmp (codec, "avc", 5)) {
    codec = "vaapih264dec";
  } else if (!g_ascii_strncasecmp (codec, "hevc", 5)) {
    codec = "vaapih265dec";
  } else if (!g_ascii_strncasecmp (codec, "jpeg", 5)) {
    codec = "vaapijpegdec";
  } else if (!g_ascii_strncasecmp (codec, "mpeg2", 5)) {
    codec = "vaapimpeg2dec";
  } else if (!g_ascii_strncasecmp (codec, "vp9", 5)) {
    codec = "vaapivp9dec";
  } else {
...
channel_thread (void *ptr)
{
  ...
  if (gst_element_link (decoder, sink) != TRUE) {
    g_printerr ("Fail to link decoder and sink.\n");
    gst_object_unref (pipeline);
    return NULL;
  }
