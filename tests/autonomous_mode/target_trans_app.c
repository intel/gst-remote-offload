/*
 *  target_trans_app.c - hddlsrc ! <decoder> ! "video/x-raw(memory:DMABuf)" !
 *                       <encoder> ! hddlsink
 *
...
static gint port = -1;
static gchar *dec_codec = NULL;
static gchar *enc_codec = NULL;
...
static GOptionEntry entries[] = {
  ...
  {"port", 'p', 0, G_OPTION_ARG_INT, &port,
      "Port to listen to.", "P"},
  {"decoder", 'd', 0, G_OPTION_ARG_STRING, &dec_codec,
      "Decoder codec to be used", "decoder"},
  {"encoder", 'e', 0, G_OPTION_ARG_STRING, &enc_codec,
      "Encoder codec to be used", "encoder"},
...
main (int argc, char *argv[])
{
  ...
  if (connection_mode == NULL || port == -1 || dec_codec == NULL ||
    enc_codec == NULL  || channel_num == -1) {
  ...
  if (!g_ascii_strncasecmp (dec_codec, "avc", 5)) {
    dec_codec = "vaapih264dec";
  } else if (!g_ascii_strncasecmp (dec_codec, "hevc", 5)) {
    dec_codec = "vaapih265dec";
  } else if (!g_ascii_strncasecmp (dec_codec, "jpeg", 5)) {
    dec_codec = "vaapijpegdec";
  } else if (!g_ascii_strncasecmp (dec_codec, "mpeg2", 5)) {
    dec_codec = "vaapimpeg2dec";
  } else if (!g_ascii_strncasecmp (dec_codec, "vp9", 5)) {
    dec_codec = "vaapivp9dec";
  } else {
    g_print ("Decoder codec may not be set correctly\n");
    return -1;
  }

   if (!g_ascii_strncasecmp (enc_codec, "avc", 5)) {
    enc_codec = "vaapih264enc";
  } else if (!g_ascii_strncasecmp (enc_codec, "hevc", 5)) {
    enc_codec = "vaapih265enc";
   } else if (!g_ascii_strncasecmp (enc_codec, "jpeg", 5)) {
    enc_codec = "vaapijpegenc";
  } else {
    g_print ("Encoder codec may not be set correctly\n");
    return -1;
  }
...
channel_thread (void *ptr)
{
  ...
  GstElement *pipeline, *source, *decoder, *encoder, *sink, *filter;
  GstBus *bus;
  GstMessage *msg;
  ...
  decoder = gst_element_factory_make (dec_codec, NULL);
  encoder = gst_element_factory_make (enc_codec, NULL);
  filter = gst_element_factory_make ("capsfilter", NULL);
  sink = gst_element_factory_make ("hddlsink", NULL);

  if (!source || !decoder || !filter || !encoder || !sink) {
    g_printerr ("Not all elements could be created.\n");
    return NULL;
  }

  /* Build the pipeline */
  gst_bin_add_many (GST_BIN (pipeline), source, decoder, filter, encoder, sink, NULL);

  if (gst_element_link (source, decoder) != TRUE) {
    g_printerr ("Fail to link source and decoder.\n");
    gst_object_unref (pipeline);
    return NULL;
  }

  if (gst_element_link (decoder, filter) != TRUE) {
    g_printerr ("Fail to link decoder and filter.\n");
    gst_object_unref (pipeline);
    return NULL;
  }

   if (gst_element_link (filter, encoder) != TRUE) {
    g_printerr ("Fail to link filter and encoder.\n");
    gst_object_unref (pipeline);
    return NULL;
  }

  if (gst_element_link (encoder, sink) != TRUE) {
    g_printerr ("Fail to link encoder and sink.\n");
    gst_object_unref (pipeline);
    return NULL;
  }
  ...
  if (data->mode == CONNECT_TCP) {
    ...
    hddl_context_src->hddl_xlink->xlink_handler = data->xlink_handler;
    hddl_context_src->hddl_xlink->channelId = data->port;

    hddl_context_sink->hddl_xlink->xlink_handler = data->xlink_handler;
    hddl_context_sink->hddl_xlink->channelId = data->port + 1;

    g_print ("Connection mode is %s. hddlsrc channel: %d, hddlsink channel: %d\n",
  ...
  g_object_set (G_OBJECT (source), "selected-target-context", hddl_context_src, NULL);
  g_object_set (G_OBJECT (sink), "connection-mode", data->mode, NULL);
  g_object_set (G_OBJECT (sink), "selected-target-context", hddl_context_sink, NULL);
  g_object_set (G_OBJECT (filter), "caps", gst_caps_from_string ("video/x-raw(memory:DMABuf)"), NULL);
