...
int main(int argc, char *argv[])
{
  ...
  gst_bin_add(GST_BIN(remoteoffloadbin), tee);
  ...
  g_signal_connect (decodebin, "pad-added", G_CALLBACK (on_pad_added), queue0);
  gst_element_link(queue0, tee);

  gst_element_link_pads (tee, "src_0", queue1, "sink");
  gst_element_link_pads (tee, "src_1", queue2, "sink");


  gst_element_link (queue1, videoconvert0);
  gst_element_link (videoconvert0, ximagesink0);

  gst_element_link (queue2, videoconvert1);
