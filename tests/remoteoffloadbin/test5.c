...
int main(int argc, char *argv[])
{
  ...
  GstElement *tee1 = gst_element_factory_make("tee", "mytee1");
  ...
  GstElement *ximagesink2 = gst_element_factory_make("ximagesink", "ximagesink2");

  GstElement *remoteoffloadbin0 =
        gst_element_factory_make("remoteoffloadbin", "myremoteoffloadbin0");
  if( !remoteoffloadbin0 )
  {
     fprintf(stderr, "Error creating remoteoffloadbin\n");
     return -1;
  }

  GstElement *remoteoffloadbin1 =
        gst_element_factory_make("remoteoffloadbin", "myremoteoffloadbin1");
  if( !remoteoffloadbin1 )
  {
     fprintf(stderr, "Error creating remoteoffloadbin\n");
     return -1;
  }
  ...
  if( comms )
  {
    g_object_set (remoteoffloadbin0, "comms", comms, NULL);
    g_object_set (remoteoffloadbin1, "comms", comms, NULL);
  }

  if( commsparam )
  {
    g_object_set (remoteoffloadbin0, "commsparam", commsparam, NULL);
    g_object_set (remoteoffloadbin1, "commsparam", commsparam, NULL);
  }
  ...
  gst_bin_add(GST_BIN(remoteoffloadbin0), tee1);
  gst_bin_add(GST_BIN(remoteoffloadbin0), queue2);
  gst_bin_add(GST_BIN(remoteoffloadbin0), queue3);
  ...
  gst_bin_add(GST_BIN(remoteoffloadbin1), videoconvert2);
  gst_bin_add(GST_BIN(remoteoffloadbin1), ximagesink2);
  gst_bin_add(GST_BIN(pipeline), remoteoffloadbin0);
  gst_bin_add(GST_BIN(pipeline), remoteoffloadbin1);

  gst_element_link (filesrc, qtdemux);
  g_signal_connect (qtdemux, "pad-added", G_CALLBACK (on_pad_added), decodebin);
  g_signal_connect (decodebin, "pad-added", G_CALLBACK (on_pad_added), tee0);

  gst_element_link_pads (tee0, "src_0", queue0, "sink");
  gst_element_link_pads (tee0, "src_1", queue1, "sink");

  gst_element_link (queue0, tee1);
  gst_element_link_pads (tee1, "src_0", queue2, "sink");
  gst_element_link_pads (tee1, "src_1", queue3, "sink");
  ...
  gst_element_link (queue1, videoconvert2);
  gst_element_link (videoconvert2, ximagesink2);
